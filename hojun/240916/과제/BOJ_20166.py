...
data = [list(input().strip()) for _ in range(N)]  # 격자 정보
...
def dfs(problem, idx, i, j): 
    if len(problem) == idx:
        return 1  # 문자열을 만들 수 있는 경우의 수 반환
    ...
    for di, dj in dir:
        ...
        if data[ni][nj] == problem[idx]:  # 다음 문자가 일치하는 경우
            count += dfs(problem, idx + 1, ni, nj)  # 재귀 호출하여 탐색

    return count  # 최종 경우의 수 반환

# 모든 문제에 대해 탐색 수행
for problem in problems:
    result = 0  # 문자열을 만들 수 있는 경우의 수
    for i, j in pos[problem[0]]:  # 첫 번째 문자의 위치 순회
        result += dfs(problem, 1, i, j)  # 첫 번째 문자에 대해 DFS 수행
